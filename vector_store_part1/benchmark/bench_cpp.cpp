...
using Elem = decltype(Record::values)::value_type;

std::vector<std::vector<Elem>> read_queries(const std::string& path) {
    std::vector<std::vector<Elem>> queries;
...
int main(int argc, char** argv) {
    ...
    std::vector<std::vector<Elem>> queries = read_queries(queries_path);
    if (queries.empty()) {
